...
function SimTimeStep( dt, positions, velocities, springs, stiffness, damping, particleMass, gravity, restitution )
{
	...
	// computes elastic and damping forces from springs
	for (var i = 0; i < springs.length; i++) {
		...
		var deltaP = positions[p1].sub(positions[p0]);
		var currentLength = deltaP.len();
		var dir = deltaP.unit();

		// (elastic) Hooke force
		var Fs = dir.mul(-stiffness * (currentLength - rest));

		// damping force
		var deltaV = velocities[p1].sub(velocities[p0]);
		var Fd = dir.mul(-damping * deltaV.dot(dir));

		// apploes particels forces
		forces[p1].inc(Fs);
		forces[p1].inc(Fd);
		forces[p0].dec(Fs);
		forces[p0].dec(Fd);
	...
	// collision management with bounding box (-1, 1)
	for (var i = 0; i < positions.length; i++) {
		for (var axis in positions[i]) {
			if (positions[i][axis] < -1 || positions[i][axis] > 1) {
				velocities[i][axis] *= -restitution;
				var bound = (positions[i][axis] < -1) ? -1 : 1;
				var excess = positions[i][axis] - bound;
				positions[i][axis] = bound - restitution * excess;
			}
		}
	}
